...
X_train = X_train.reshape(X_train.shape[0], 28,28,1).astype('float32') / 255   # shape(60000, 28,28,1)   데이터의 개수가 6만개  
...
model.fit_generator(data_generator.flow(X_train, Y_train, batch_size=200), 
                    steps_per_epoch=300,   # 몇배로 증폭시킬것인가
                    # (len(X_train)/200) * 200 * steps_per_epoch 개로 데이터 증폭
                    epochs = 200,   # 반복횟수
