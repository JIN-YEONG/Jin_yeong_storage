...
model.add(LSTM(32, input_shape=(2,2)))   # inpurt_shape를 변경할 경우 두값의 곱은 항상 같아야 한다.
# Dense 결합
model.add(Dense(5, activation='relu'))
# model.add(Dense(5, activation='relu'))
# model.add(Dense(5, activation='relu'))
model.add(Dense(4))   # 4개의 출력 다:다 모델
...
x_predict = split_10(x,size)
x_predict = x_predict[:,0:4]
x_predict = x_predict.reshape((x_predict.shape[0], x_predict.shape[1]//2,2))
