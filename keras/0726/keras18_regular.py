...
model.add(Dense(1000, input_dim=3, activation='relu', kernel_regularizer=regularizers.l2(0.07)))   
...
model.add(Dense(1000,kernel_regularizer=regularizers.l1(0.1)))
model.add(BatchNormalization())   # 정규화 과정
model.add(Dense(1000,kernel_regularizer=regularizers.l1(0.001)))
model.add(Dropout(0.5))   # 노드를 50%를 줄인다.
model.add(Dense(1000))
model.add(BatchNormalization())   # 정규화 과정
model.add(Dropout(0.4))   # 노드를 40%를 줄인다.
model.add(Dense(1000,kernel_regularizer=regularizers.l1(0.05)))
model.add(Dropout(0.2))   # 노드를 20%를 줄인다.
model.add(Dense(1000,kernel_regularizer=regularizers.l1(0.5)))
model.add(Dense(1000,kernel_regularizer=regularizers.l1(0.003)))
model.add(Dense(1000,kernel_regularizer=regularizers.l1(0.08)))
model.add(BatchNormalization())   # 정규화 과정
model.add(Dense(1000))
...
model.fit(x_train,y_train,epochs=100, batch_size=32, validation_data=(x_val, y_val),callbacks=[early_stopping],verbose=3)   # validation_data = 검증을 위한 데이터 셋
